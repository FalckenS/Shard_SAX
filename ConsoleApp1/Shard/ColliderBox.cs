...
internal class ColliderBox : Collider
{
    ...
    internal override bool checkCollision(Vector3 rayOrigin, Vector3 rayDirection)
    {
        ...
        // each axis where the ray is "inside" the box along that axis.
        
        float txmin = (MinAndMaxX[0] - rayOrigin.X) / rayDirection.X;
        float txmax = (MinAndMaxX[1] - rayOrigin.X) / rayDirection.X;
        if (txmax < txmin)
        {
            (txmin, txmax) = (txmax, txmin);
        }

        float tymin = (MinAndMaxY[0] - rayOrigin.Y) / rayDirection.Y;
        float tymax = (MinAndMaxY[1] - rayOrigin.Y) / rayDirection.Y;
        if (tymax < tymin)
        {
            (tymin, tymax) = (tymax, tymin);
        }

        // If the intervals on both axes don't overlap, the ray can't be intersecting the box (no collision)
        ...
        float tzmin = (MinAndMaxZ[0] - rayOrigin.Z) / rayDirection.Z;
        float tzmax = (MinAndMaxZ[1] - rayOrigin.Z) / rayDirection.Z;
        if (tzmax < tzmin)
        {
            (tzmin, tzmax) = (tzmax, tzmin);
        }

        // If the intervals don't overlap, the ray can't be intersecting the box (no collision)
        if (tmin > tzmax || tzmin > tmax) return false;

        // Ensures the intersection is not completely behind the ray
        return tmax >= 0;
    }
